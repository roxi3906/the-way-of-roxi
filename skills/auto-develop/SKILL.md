...
Capture the delivery start time now, with an explicit timezone, in a sibling private `{task-summary}-report.json` presentation file with `status: "running"`. Preserve it across continuations. Keep report timing, delivery metadata, valuable stage outcomes, and review finding-to-fix records in this separate file as verified evidence becomes available; never add presentation fields to the decision ledger. Read [references/html-report.md](references/html-report.md) for its input format and report-service lifecycle. Start one task-owned loopback Node service once both inputs are valid, and share its live URL. Update presentation JSON atomically after meaningful outcomes so the open report refreshes automatically. The service also maintains an offline HTML snapshot.
...
Before writing a terminal response for a delivery or risk-gate pause, re-read this Skill, [references/execution-report.md](references/execution-report.md), and that delivery's private ledger. Reconcile every current decision object with the final status records and its fully expanded timeline entry. The HTML report is invalid if the ledger read-back record, any material decision, any required phase, or any decision-detail field is missing. Render the decision tree as a timeline in the current session language while keeping the fixed JSON keys and technical literals unchanged. A message that does not start or continue a delivery receives an ordinary response under the active session rules without reading an earlier ledger or rendering an execution report.

Render the final report as one self-contained, minimal HTML file using [references/html-report.md](references/html-report.md), preserving the evidence contract in [references/execution-report.md](references/execution-report.md). Its four main chapters are task overview with elapsed time and PR information, valuable intermediate stages, code-review findings with corresponding repairs, and a decision timeline with every decision's complete details. Keep report content expanded in normal page flow without collapsible content or internal scrolling. The decision outline is the only navigation exception: show every node in a sticky right sidebar, with local jump links, current-node highlighting, and a scrollable list when needed. On narrow screens place the expanded outline above the timeline. Visually distinguish recommended options from alternatives and label the actual selection independently. Use [scripts/render-execution-report.mjs](scripts/render-execution-report.mjs) when Node.js is available. The renderer reads the original decision ledger without changing it. Attach absolute links to the HTML report and private decision-tree JSON in every terminal delivery response, with a concise outcome and any blocker; the full commands, audit records, findings, fixes, and verification evidence belong in the HTML. Never infer successful state from an attempted command. A risk-gate pause must still render the ledger path and entries accumulated through the blocker.

Fill the report from verified delivery evidence:

- **Overview**: one short outcome sentence, concise task analysis, verified repository identity, task branch, absolute working directory, recorded timing, and verified PR number/URL/state/refs. Record these in presentation fields, not the decision schema. Group metadata into compact rows; session metadata and the ledger path belong in the audit block.
- **Stages**: one entry per valuable outcome, stating what was learned or delivered and its durable evidence. Record actual verification commands and final results; omit routine narration.
- **Review**: pair every finding with severity, evidence, applied fix or unresolved disposition, repair verification, and re-review outcome. Capture `reviewedAt`, `fixedAt`, and `verifiedAt` at the corresponding events with timezones; record the latest review time at report level even when no findings exist. Leave unverified timestamps empty rather than inferring them. Do not invent findings to fill the section.
- **Decisions**: read every ledger entry in order and display all options, descriptions, IDs, type, parent, evidence, rationale, risk, involvement, reversibility, and outcome. Put original creation/update times above the decision title; use separate recommendation and selection tags. Render the ledger's `reason` once in a pale green explanation block below the selected option's description. Without a selection, retain the reason as a regular decision field. Preserve schema/session/task metadata in the audit block, and show missing values honestly.

Use spacing rather than divider lines or decorative separator characters; the decision timeline uses thin pale green segments connecting larger round Marrs Green nodes, ending at the last node. Use Marrs Green accents, large bold chapter numbers, and pale green chapter banners spanning the report width with a subtle lower shadow that remains visible while sticky. Keep text left-aligned, group overview metadata into readable rows, avoid forced breaks within values and far-apart label/value placement, and let long technical literals wrap only when needed. Chapter titles and the current stage, review, decision, or audit block title stick in separate vertical layers while scrolling, without obscuring each other.

At every heading level, show the lower shadow only while actually pinned. Remove it before pinning, after release at the container boundary, and when scrolling back. Use the bundled scroll/layout enhancement for shadows and the current outline entry; all content and local jump links work without network access.
...
Keep the terminal chat reply to one short outcome sentence and one compact line of links to the live report, offline HTML snapshot, original decision JSON, and verified draft PR when available. Verify service status before linking it; use the offline snapshot if the service has stopped. Add a short sentence for a blocker, material residual risk, or necessary user action; do not bury those in the report. After verified draft-PR creation, also include the short post-merge cleanup reminder. Do not repeat implementation lists, test counts or commands, review details, decision summaries, or paths already in the HTML. Use no report-style headings or recap bullets by default. Expand only when the user asks for detail, explicitly requests text-only output, or the report cannot be delivered; disclose any missing artifact rather than linking or claiming it.

Keep the worktree path and task branch in the HTML. After verified draft-PR creation, append this reminder once in chat: “PR 合并后，可以让我清理本地开发工作树和任务分支，以释放资源。” Do not ask the user to invoke `auto-develop` for cleanup. A later cleanup request follows the user's existing workflow and also stops this task's report service. Use the documented service status/stop commands; do not terminate unrelated processes.
