...
- configured tracking-item binding or creation when the 90% gate passes, followed by automatic binding, creation, progress updates, and legal transitions for every valuable intermediate outcome.
...
Use the configured tracking Skill, CLI, API, or monitor, preserving its discovery, authentication, ownership, workflow, pagination, idempotency, and read-back rules. Prefer the task's verified existing binding, then its explicitly selected platform, then the repository's configured destination. TAPD is one supported integration, not a required platform. Do not create parallel records in another system; resolve conflicting destinations through the risk gate.
...
Use a hybrid child model: the parent receives the full lifecycle, and every valuable intermediate outcome must have a verified tracking item. Before substantive work in each stage, apply [references/tracking-outcomes.md](references/tracking-outcomes.md) to identify outcomes, bind or create their items, and follow them through progress, blocking, resumption, and evidence-based completion. An outcome need not ship separately: a reviewable research conclusion, design decision, compatibility matrix, or review remediation qualifies when its scope and acceptance criteria are clear. Create or bind before work, not after its artifact exists. Routine operations remain parent progress; internal steps for the same outcome share one item. Child synchronization failures remain explicit unsynchronized work, never silent omissions.
...
Use the Marrs Green visual baseline: spacing instead of dividers (except timeline connectors), large bold chapter numbers, full-width pale green banners, left-aligned compact rows, and wrapping only as needed. Keep content expanded in page flow. Show the bound parent immediately below the summary under Parent task. Show Related child tasks beneath each corresponding stage, review, or key decision heading, using `trackingItems` and presentation-only `decisionTracking` in the HTML report contract. Label the actual platform Workflow status separately from delivery/review status; both task titles and workflow statuses must link to their verified work items. Preserve verified titles, platform names, actual statuses, and unavailable-link reasons in both live and offline reports. The sticky right decision outline is the sole local-navigation/internal-scroll exception; narrow screens place it above the timeline. Chapter and current block titles stick in separate layers without overlap. At every level, the bundled enhancement adds a lower shadow only while actually pinned and removes it before pinning, on reverse scroll, and at container release. Content and local anchors remain usable offline.

Before successful closeout, reconcile every planned or discovered valuable outcome against its verified item, evidence, current status, and any unsynchronized action; catch missing items and incomplete transitions without duplicating completed work. Then synchronize `delivery closeout` as completed with commit, push, draft-PR, ledger-reconciliation, and report-preflight evidence, then read it back. On a risk-gate pause, mark `delivery closeout` as `blocked` only if closeout started and the integration accepted and returned that event; otherwise record the attempt as unsynchronized and leave closeout incomplete. Complete closeout only after the blocker is resolved. Include aggregate synchronization, per-event payloads, and hybrid-child results; attempted writes never establish success. A paused report retains the ledger path and every decision through the blocker.
