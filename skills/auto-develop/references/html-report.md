...
```json
{
  ...
  "trackingItems": [
    { "platform": "Configured tracker", "title": "Report delivery", "url": "https://tracker.example.invalid/items/parent", "status": "In progress" }
  ],
  "stages": [
    {
      ...
      "trackingItems": [
        { "platform": "Configured tracker", "title": "Schema compatibility verification", "url": "https://tracker.example.invalid/items/child", "status": "Completed" }
      ]
    }
  ],
  "reviews": [
...
Place the bound parent in report-level `trackingItems`. The template displays it immediately below the summary, before repository metadata, under **Parent task**. Reserve this position for the parent rather than mixing unrelated child outcomes into it. Put child items in `stages[].trackingItems` and remediation items in `reviews[].trackingItems`; the template shows **Related child tasks** directly below each node heading, before its evidence or explanation.

For a key decision-tree node, add a presentation-only `decisionTracking` entry with its immutable `decisionId` and `trackingItems` array. Each ID must refer to a real ledger decision and appear at most once; one entry can contain several child items. The renderer rejects unknown or duplicate IDs rather than attaching a link to the wrong node. Keep this mapping in report JSON, never in the fixed decision ledger. Reuse the same verified item URL and status when one outcome is shown in several places.

Every created, reused, restored, advanced, blocked, or completed child must appear beside its corresponding stage, review finding, or decision node. If no existing node represents it, add the actual outcome as a stage; do not hide it in the summary or plain-text audit. These optional arrays preserve older reports without tracking data.

Each item has nonempty `platform`, user-readable `title`, actual `status`, and a `url` resolved through the configured adapter and verified against the binding. The HTML shows the platform and task title alongside an explicit **Workflow status** label. Both the task title and its current platform status link to the same verified work item in live and offline reports, with escaped labels and safe new-tab behavior. Display the actual last verified platform status, not the report's running/completed state or the review's fixed status; a failed transition retains the last verified status and explains the pending synchronization in `reason`. Never invent URLs or use internal IDs as labels. If the item exists but link resolution fails, use `url: ""` and a nonempty `reason`; it renders as text with that reason. A known link can remain present alongside a `reason` explaining an unsynchronized update. Unsupported or unavailable tracking belongs in context/audit with its reason, not in a fabricated item. Reconcile links and statuses at each report refresh and closeout; syntax validation alone does not establish that a remote item or status was verified.
...
Use the current session language for all human-readable input. The renderer provides English and Simplified Chinese interface labels according to `session.language`; for another language, pass a `labels` object translating every key in the renderer's `copy.en` map. `labels` also allows explicit destination wording overrides. Tracking headings use `parentTask`, `childTasks`, and `workflowStatus`; older custom translations fall back to their existing `tracking` and `outcome` labels when those new keys are absent. Technical literals and status enum values remain unchanged. For ledger text recorded in an earlier language, use the exported `renderExecutionReport` function with a display-only clone whose human-readable values have been translated; preserve all fixed keys, IDs, references, timestamps, technical literals, and decision ordering. Reconcile that clone with the original ledger before rendering and never persist it over the original. The standard CLI renders the ledger as recorded and does not translate content.
...
Open the actual HTML using the host's preview or browser tools when available. Check desktop and narrow layouts, compact overview, complete context/audit content, every option and timeline entry, timestamps above titles, independent tags, and the absence of unrelated dividers or controls. Verify timeline segments join consecutive node centers and stop at the final node. Test project-management link destinations, unavailable-link explanations, live updates and offline link rendering. Test every outline link, keyboard navigation, current-entry highlighting, long-list visibility, sidebar boundaries, and unobscured jump targets; verify native links also work without JavaScript. Scroll within multiple chapters and blocks to verify their headings stick in separate layers, gain shadows only while pinned, and release at content boundaries. Compare displayed session metadata, IDs, types, parent references, options, and original timestamps against parsed JSON. Include a choice that differs from the recommendation. Verify report generation left source JSON unchanged. State any unavailable visual check precisely; do not call uninspected output visually verified.
