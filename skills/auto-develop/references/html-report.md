...
Include `trackingItems` in the report overview for the bound parent and any tracked outcome not represented elsewhere. Put each stage's corresponding items in `stages[].trackingItems`, and a finding's remediation items in `reviews[].trackingItems`. Every created, reused, restored, advanced, blocked, or completed item must have a visible entry next to its outcome or in the overview. Do not bury platform links solely in the plain-text audit. These optional arrays preserve compatibility with older reports and do not change the decision-ledger schema.

Each item has nonempty `platform`, user-readable `title`, actual `status`, and a `url` resolved through the configured adapter and verified against the binding. The HTML renders HTTP(S) URLs as native clickable links in both live and offline reports, with escaped labels and safe new-tab behavior. Never invent URLs or use internal IDs as labels. If the item exists but link resolution fails, use `url: ""` and a nonempty `reason`; it renders as text with that reason. A known link can remain present alongside a `reason` explaining an unsynchronized update. Unsupported or unavailable tracking belongs in context/audit with its reason, not in a fabricated item. Reconcile links and statuses at each report refresh and closeout; syntax validation alone does not establish that a remote item or status was verified.
