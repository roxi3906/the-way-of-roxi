...
For each delivery, `auto-develop` synchronizes preparation, technical research, solution design, implementation, verification, code review, and closeout through the user's configured project-management integration. The bound parent records the complete lifecycle. Every valuable intermediate outcome with a reviewable output and acceptance criterion is bound or created before work starts, including research reports, design decisions, verification results, and review remediation. Items receive progress, blocked/resumed updates, and legal evidence-based transitions throughout execution; routine operations share their existing item. Completed outcomes are restored without duplicates across continuations. TAPD is one supported integration; the workflow uses the user's configured platform and verifies every write.
...
- Binds or creates children for every valuable planned or discovered stage outcome before work, then follows progress, blockers, resumption, and verified legal transitions; routine operations remain existing-item progress
- Completes code-bearing children only after acceptance and covering successful commits, and non-code phase children from durable accepted evidence without requiring a commit; restores completed same-delivery bindings without duplicate writes
...
- Assigns every change a unique, increasing number across all modes within the task; preserves numbers through revisions and mode switches, never reuses retired numbers, and accepts adjustment requests by number
...
Focused outcome lifecycle probes run with `npm run verify:codex -- --case auto-develop-outcomes` and `--case tapd-sync-outcomes`. Each asks the consuming agent for decisions on planned research and verification, existing-item reuse, blocked/resumed work, code versus non-code completion, completed-outcome restoration, ambiguous workflows, and failed read-back. The generic probe uses a configured non-TAPD tracker. These isolated simulations perform no platform writes and do not establish live integration correctness.
